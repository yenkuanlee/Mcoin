...
contract Ballot {
   ...
   struct Voter {
    ...
    function Ballot(uint _numProposals,uint _totalTicket) public {
	NProposals = _numProposals;
        chairperson = msg.sender;
        voters[chairperson].weight = _totalTicket;
    ...
    function vote(uint toProposal, uint VC) public {
        Voter storage sender = voters[chairperson];
    ...
    function GetRemainVoteWeight() public constant returns (uint RVW) {
	Voter storage sender = voters[chairperson];
	RVW = sender.weight;
    }
}
